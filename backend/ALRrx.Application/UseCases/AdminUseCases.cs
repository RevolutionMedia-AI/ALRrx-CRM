...
public sealed class ResetUserPasswordUseCase
{
    ...
    public ResetUserPasswordUseCase(IUserRepository users, IAuthService auth, IAuditLogRepository audit, IEmailService email)
    {
        _users = users;
        _auth = auth;
        _audit = audit;
    ...
    public async Task<PasswordResetResult> ExecuteAsync(int userId, int performedBy, string? ip, CancellationToken ct = default)
    {
        var user = await _users.GetByIdAsync(userId, ct) ?? throw new InvalidOperationException("User not found");
        var tempPassword = GenerateTemporaryPassword();
        var hash = _auth.HashPassword(tempPassword);
        await _users.ResetPasswordAsync(userId, hash, ct);

        await _audit.LogAsync(new UserAuditLog
        {
            UserId = userId,
            Action = nameof(AuditAction.PasswordReset),
            PerformedBy = performedBy,
            IpAddress = ip,
        }, ct);

        var emailResult = await _email.SendPasswordResetAsync(user.Email, user.FullName, tempPassword, ct);
        await AdminAuditHelpers.LogEmailFailureAsync(_audit, userId, performedBy, emailResult, ip, ct);
